...
fn derive_taproot_key_v028(master_key: &bitcoin_v028::util::bip32::ExtendedPrivKey) -> Result<bitcoin_v028::secp256k1::PublicKey, Box<dyn Error>> {
    let secp = bitcoin_v028::secp256k1::Secp256k1::new();
    let path = bitcoin_v028::util::bip32::DerivationPath::from_str("m/86'/0'/0'/0/0")?;
...
pub fn generate_all_addresses() -> Result<(), Box<dyn Error>> {
    ...
    let taproot_pubkey = derive_taproot_key_v028(&root_key_v028)?;
    ...
    let derivation_path_legacy = DerivationPath::from_str("m/44'/0'/0'/0/0")?;
    ...
    let derivation_path_segwit = DerivationPath::from_str("m/84'/0'/0'/0/0")?;
    ...
    let derivation_path_p2sh = DerivationPath::from_str("m/49'/0'/0'/0/0")?;
    ...
    let derivation_path_p2wsh = DerivationPath::from_str("m/48'/0'/0'/2/0")?;
